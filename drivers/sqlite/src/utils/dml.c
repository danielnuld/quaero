...
/* Append `name` as a double-quoted SQL identifier, doubling embedded quotes. */
...
/* True when a value of neutral type `type` is a bare numeric literal safe to emit
   UNQUOTED (int/float only; a plain optionally-signed decimal, so an unquoted
   value can never carry a quote/semicolon — no injection risk). Anything else is
   quoted. SQLite accepts quoted numerics too, but this keeps DML consistent with
   the other drivers and stores numeric affinity correctly. `bool` is excluded. */
...
/* Append `[schema.]table`, each part quoted. */
...
/* Append `col = val` / `col IS NULL` terms joined by AND (the WHERE body). */
...
char *sqlite_build_dml_sql(dbc_dml_kind kind, const dbc_dml_row *row)
{
    ...
    /* Refuse statements that would touch every row or set nothing. */
