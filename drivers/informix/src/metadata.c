 ...
 * syscolumns.coltype covers the base type names; the primary-key flag is detected
 * from the 'P' constraint's index (so composite keys mark every member column).
 * Declared length/precision and default values are still omitted here — the full
 * type with lengths is reconstructed by get_ddl (ddl.c). */

/* True if s is a safe Informix identifier (used unquoted as a db qualifier).
   Shared with ddl.c via internal.h. */
int ifx_is_safe_ident(const char *s)
{
...
/* Owned `'<escaped>'` SQL string literal (single quotes doubled), or NULL OOM.
   Shared with ddl.c via internal.h. */
char *ifx_quote_literal(const char *value)
{
    size_t extra = 0;
                              ...
                              const char *table, dbc_result **out)
{
    ...
    char *table_lit = ifx_quote_literal(table);
    if (table_lit == NULL) {
    ...
    static const char *const k_select =
        ...
        "CAST(NULL AS VARCHAR(255)) AS dflt_value, "
        /* pk: 1 when the column participates in the table's PRIMARY KEY. The key
           columns are the parts of the index backing the 'P' constraint; a part
           is negative for a descending column, so match on ABS(part). Without
           this the frontend treats every Informix table as read-only. */
        "CASE WHEN EXISTS ("
        "SELECT 1 FROM %ssysconstraints k, %ssysindexes x "
        "WHERE k.tabid = t.tabid AND k.constrtype = 'P' AND x.idxname = k.idxname "
        ...
        "ABS(x.part15),ABS(x.part16))) THEN 1 ELSE 0 END AS pk "
        "FROM %ssyscolumns c, %ssystables t "
        "WHERE t.tabname = %s AND t.tabid = c.tabid ORDER BY c.colno";

    size_t n = strlen(k_select) + 4 * strlen(qualifier) + strlen(table_lit) + 1;
    char *sql = malloc(n);
    if (sql == NULL) {
        free(table_lit);
        return DBC_ERR_NOMEM;
    }
    snprintf(sql, n, k_select, qualifier, qualifier, qualifier, qualifier,
             table_lit);
    free(table_lit);
