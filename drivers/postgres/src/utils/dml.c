...
/* Append `name` as a double-quoted identifier, doubling embedded double quotes. */
...
/* Append `val` as a SQL literal: NULL keyword for NULL, else a single-quoted
   string with the standard-SQL escape (double an embedded single quote). */
...
/* True when a value of neutral type `type` is a bare numeric literal safe to emit
   UNQUOTED. Only int/float columns qualify, and the value must be a plain
   optionally-signed decimal — so an unquoted value never contains a quote and
   there is no injection risk. Anything else (a non-numeric value, or a
   text/date/blob/bool column) is quoted. `bool` is deliberately excluded: a
   PostgreSQL boolean column happily coerces the quoted 't'/'f'/'true' forms. */
...
/* Append a set value: NULL keyword, a bare numeric literal for a numeric column,
   or an escaped string literal otherwise. */
